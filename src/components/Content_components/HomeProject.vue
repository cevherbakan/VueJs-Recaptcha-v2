      ...
      <div class="row">
        ...
        >
          <div class="feature-block">
            ...
            <router-link :to="'/project_detail?project_id=' + item.id">
              <img
                v-if="item.project_images"
                :src="img_base_url + item.project_images.img_url"
                alt="img"
                class="img-fluid"
              />
            </router-link>
            ...
            <div class="content-blog">
              <h4>
                <router-link
                  :to="'/project_detail?project_id=' + item.id"
                  v-html="'<div>' + item.project_name + '</div>'"
                >
                </router-link>
              </h4>
              <p
      ...
      .then((response) => {
        ...
        for (var i = 0; i < this.result.length; i++) {
          ...
          for (var j = 0; j < this.images.length; j++) {
            if (
              this.result[i]["id"] == this.images[j]["project_id"] &&
              key == true
            ) {
