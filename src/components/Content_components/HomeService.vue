          ...
          <div class="feature-block">
            ...
            <router-link :to="'/service_detail?service_id=' + item.id">
              <img
                ...
                :src="img_base_url + item.product_images.img_url"
                alt="img"
                class="img-fluid"
              />
            </router-link>
            ...
            <div class="content-blog">
              <h4>
                <router-link
                  :to="'/service_detail?service_id=' + item.id"
                  v-html="'<div>' + item.product_name + '</div>'"
                >
                </router-link>
              </h4>
              <p
...
export default {
  ...
  created: function () {
    let dataUrl =
      store.state.base_url + "Product/getByNumberProducts.php?key=123";
    axios
      ...
      .then((response) => {
        ...
        for (var i = 0; i < this.result.length; i++) {
          ...
          for (var j = 0; j < this.images.length; j++) {
            if (
              this.result[i]["id"] == this.images[j]["product_id"] &&
              key == true
            ) {
